...
% Header
\noindent
\textbf{\LARGE{\Sexpr{as.character(nha_siteName)}}}\\
  ...
  \small
   \centering
    \begin{tabular}{p{3in}cccccc}
     \hline
     Species or Natural Community Name &  & Global\tnote{1} & State\tnote{1} & PA Legal Status\tnote{1} & LastSeen	& Quality\tnote{2} \\
     \hline
     <<label=speciestable, echo=FALSE, results='asis'>>=
     NHAspecies$SNAME_or <- NHAspecies$SNAME
     NHAspecies$SNAME[NHAspecies$SENSITIVE =="Y"] = paste(NHAspecies$SNAME,"\\tnote{3} ") 
     for(i in 1:nrow(NHAspecies)){
       cat(paste0(NHAspecies$SCOMNAME[i]," (\\textit{",NHAspecies$SNAME[i],"}) ", "&", "\\includegraphics[width=0.15in]{",NHAspecies$Images[i],"}", "&", NHAspecies$GRANK[i], "&", NHAspecies$SRANK[i], "&", paste0(NHAspecies$SPROT[i]," (",NHAspecies$PBSSTATUS[i],")"), "&", NHAspecies$LASTOBS[i], "&", NHAspecies$EORANK[i],"\\\\" ) )
     }
