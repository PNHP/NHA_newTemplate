   ...
   \centering
    \begin{tabular}{p{3in}cccccc}
     ...
     for(i in 1:nrow(NHAspecies)){
       cat(paste0(NHAspecies$SCOMNAME[i]," (\\textit{",NHAspecies$SNAME[i],"}) ","&", NHAspecies$ELEMENT_TYPE[i], "&", NHAspecies$GRANK[i], "&", NHAspecies$SRANK[i], "&", paste0(NHAspecies$SPROT[i]," (",NHAspecies$PBSSTATUS[i],")"), "&", NHAspecies$LASTOBS[i], "&", NHAspecies$EORANK[i],"\\\\" ) )
     }
@
     \\ \hline                      
    \end{tabular}
