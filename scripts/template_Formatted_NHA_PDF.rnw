  ...
  \small
   \centering
    \begin{tabular}{p{3in}cccccc}
     ...
     #NHAspecies$OrderVec <- as.character(NHAspecies$OrderVec)
     ...
     for(i in 1:nrow(NHAspecies)){ 
       # set up sensitive species id vector
       ...
       if(NHAspecies$SENSITIVE[i]=="Y"){
         ...
         speciesstring <- paste("Sensitive Species of Concern",SSOCid[j],"\\tnote{3} ") 
       } else {
